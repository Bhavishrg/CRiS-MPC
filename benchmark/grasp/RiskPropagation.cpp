// benchmark/grasp/RiskPropagation.cpp
//
// NPH benchmark for DCC/GraSP-style risk propagation over ring values.
//
// Usage:
//   ./run.sh RiskPropagation --protocol nph --num-parties 2 --graph-size 10000 --num-iters 10
//
...
struct CircuitData {
    ...
    std::vector<wire_t> risk_in;
    std::vector<wire_t> inv_total_in;
    std::vector<std::vector<wire_t>> amount_in;
...
struct PlainInputs {
    ...
    std::vector<T> risk;
    std::vector<T> inv_total;
    std::vector<std::vector<T>> amount_by_party;
    std::vector<T> expected_risk;
...
static void printUsage(const char* prog) {
    std::fprintf(stderr,
        ...
        "[--num-iters <r>] [--delta-a <a>] [--delta-b <b>] [--seed <s>] [--pking] "
...
static Args parseArgs(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        } else if (std::strcmp(argv[i], "--delta-a") == 0 && i + 1 < argc) {
            a.delta_a = static_cast<T>(std::strtoull(argv[++i], nullptr, 10));
        } else if (std::strcmp(argv[i], "--delta-b") == 0 && i + 1 < argc) {
            a.delta_b = static_cast<unsigned>(std::strtoul(argv[++i], nullptr, 10));
    ...
    if (a.protocol_name != "nph" &&
        ...
        std::fprintf(stderr, "RiskPropagation is currently NPH-only.\n");
        printUsage(argv[0]);
        std::exit(1);
    }
...
static T ringPower(T base, unsigned exponent) {
    T result = 1;
    while (exponent != 0) {
        if ((exponent & 1U) != 0) {
            result *= base;
...
static T iterationDelta(T a, unsigned b, size_t iteration) {
    const T denominator = ringPower(static_cast<T>(iteration), b);
    return denominator == 0 ? T{0} : a / denominator;
}
...
static CircuitData generateCircuit(const grasp::GraSPPartitionPlan& plan,
    ...
    cd.risk_in = makeInputVector(c, num_vertices, P1);
    cd.inv_total_in = makeInputVector(c, num_vertices, P1);
    cd.amount_in.resize(plan.num_parties);
    ...
    for (int p = 0; p < num_compute_parties; ++p) {
        ...
        cd.amount_in[static_cast<size_t>(p)] =
            makeInputVector(c, sub.edge_rows.size(), P1);
    ...
    wire_t zero = c.addGate(GateType::kSub, cd.risk_in[0], cd.risk_in[0]);
    std::vector<wire_t> risk = cd.risk_in;
    std::vector<std::vector<wire_t>> inv_total_by_party =
        c.addAmorPermShareGate(cd.inv_total_in,
                               num_compute_parties,
                               cd.gid_decompose);

    for (int iter = 0; iter < num_iters; ++iter) {
        const T delta = iterationDelta(delta_a,
                                       delta_b,
                                       static_cast<size_t>(iter) + 1);
        const T one_minus_delta = T{1} - delta;

        std::vector<std::vector<wire_t>> risk_by_party =
            c.addAmorPermShareGate(risk, num_compute_parties, cd.gid_decompose);

        std::vector<wire_t> next_risk(num_vertices, zero);
        ...
        for (int p = 0; p < num_compute_parties; ++p) {
            ...
            std::vector<wire_t> risk_in_vbar_order =
                c.addLocalPermGate(risk_by_party[static_cast<size_t>(p)],
                                   pc.sigma_decompose.pub,
                                   false);
            std::vector<wire_t> risk_vertices_vbar =
                takePrefix(risk_in_vbar_order, vbar_count);
            std::vector<wire_t> risk_vertices_compute(vbar_count);
            for (size_t k = 0; k < vbar_count; ++k) {
                risk_vertices_compute[k] = risk_vertices_vbar[vertex_reorder[k]];
            }
            std::vector<wire_t> sub_risk =
                appendZeros(risk_vertices_compute,
                            sub.edge_rows.size(),
                            zero);
            ...
            std::vector<wire_t> sub_amount(vbar_count, zero);
            sub_amount.insert(sub_amount.end(),
                              cd.amount_in[static_cast<size_t>(p)].begin(),
                              cd.amount_in[static_cast<size_t>(p)].end());
            std::vector<wire_t> source_amount =
                addDccPermShThenPublicPerm(
                    ...
                    p,
                    cd.gid_source[static_cast<size_t>(p)]);
            for (size_t row = 0; row < subgraph_size; ++row) {
                source_data[row] = c.addGate(GateType::kMul,
                                             source_data[row],
                                             source_amount[row]);
            }
            ...
            std::vector<wire_t> inv_total_in_vbar_order =
                c.addLocalPermGate(inv_total_by_party[static_cast<size_t>(p)],
                                   pc.sigma_decompose.pub,
                                   false);
            std::vector<wire_t> inv_total_vertices_vbar =
                takePrefix(inv_total_in_vbar_order, vbar_count);
            std::vector<wire_t> inv_total_vertices_compute(vbar_count);
            for (size_t k = 0; k < vbar_count; ++k) {
                inv_total_vertices_compute[k] =
                    inv_total_vertices_vbar[vertex_reorder[k]];
            }

            for (size_t j = 0; j < owned; ++j) {
                const size_t pos = owned_positions[j];
                wire_t normalized = c.addGate(GateType::kMul,
                                              inv_total_vertices_compute[pos],
                                              gathered[pos]);
                wire_t retained = c.addCGate(GateType::kCMul,
                                             risk_vertices_compute[pos],
                                             one_minus_delta);
                wire_t incoming = c.addCGate(GateType::kCMul,
                                             normalized,
                                             delta);
                next_risk[sub.owned_vertices.begin + j] =
                    c.addGate(GateType::kAdd, retained, incoming);
            }
        }

        risk = std::move(next_risk);
    }
...
static std::vector<T> expectedRiskPropagation(
    const graphiti::GraphList& graph,
    size_t num_vertices,
    int num_iters,
    T delta_a,
    unsigned delta_b,
    const std::vector<T>& initial_risk,
    const std::vector<T>& inv_total,
    const std::vector<T>& edge_amount) {
    std::vector<T> risk = initial_risk;
    ...
    for (int iter = 0; iter < num_iters; ++iter) {
        ...
        for (size_t e = 0; e < edge_amount.size(); ++e) {
            const size_t row = num_vertices + e;
            const size_t src = static_cast<size_t>(graph.src[row]);
            const size_t dst = static_cast<size_t>(graph.dst[row]);
            if (src < num_vertices && dst < num_vertices) {
                messages[dst] += edge_amount[e] * risk[src];
            }
        }
        std::vector<T> next(num_vertices);
        for (size_t v = 0; v < num_vertices; ++v) {
            next[v] = one_minus_delta * risk[v] +
                      delta * inv_total[v] * messages[v];
        }
        risk = std::move(next);
    }
...
static PlainInputs makePlainInputs(size_t num_vertices,
    ...
    in.risk.resize(num_vertices);
    in.inv_total.resize(num_vertices);
    for (size_t v = 0; v < num_vertices; ++v) {
        in.risk[v] = static_cast<T>(1 + ((seed + 17 * v) % 11));
        in.inv_total[v] = static_cast<T>(1 + ((seed + 29 * v) % 5));
    }

    std::vector<T> edge_amount(num_edges);
    for (size_t e = 0; e < num_edges; ++e) {
        edge_amount[e] = static_cast<T>(1 + ((seed + 37 * e) % 13));
    }

    in.amount_by_party.resize(static_cast<size_t>(num_compute_parties));
    for (int p = 0; p < num_compute_parties; ++p) {
        const auto& rows = in.plan.subgraphs[static_cast<size_t>(p)].edge_rows;
        auto& amounts = in.amount_by_party[static_cast<size_t>(p)];
        amounts.reserve(rows.size());
        for (size_t row : rows) {
            amounts.push_back(edge_amount[row - num_vertices]);
        }
    }

    if (make_expected) {
        in.expected_risk = expectedRiskPropagation(in.graph,
                                                   num_vertices,
                                                   num_iters,
                                                   delta_a,
                                                   delta_b,
...
static void benchmark(const Args& args) {
    ...
    std::printf("  delta_a       : %" PRIu64 " (ring value)\n", args.delta_a);
    std::printf("  delta_b       : %u\n", args.delta_b);
    ...
    if (!is_helper && pid == P1) {
        online.setInputs(cd.risk_in, plain.risk);
        online.setInputs(cd.inv_total_in, plain.inv_total);
        for (size_t p = 0; p < cd.amount_in.size(); ++p) {
            online.setInputs(cd.amount_in[p], plain.amount_by_party[p]);
        }
    }
    ...
    if (!is_helper && plain.has_expected) {
        checked = true;
        ok = sameVector(out, plain.expected_risk);
    }

    std::printf("[P%d] DCC risk propagation correctness: %s%s\n",
                pid,
    ...
    if (!is_helper && (args.graph_size <= 40 || (checked && !ok))) {
        ...
        if (plain.has_expected) {
            printVector("expected", pid, plain.expected_risk);
        }
    }
    ...
    output_doc["details"] = {
        ...
        {"arithmetic", "uint64_ring_no_fixed_point_truncation"},
        {"seed", args.seed},
