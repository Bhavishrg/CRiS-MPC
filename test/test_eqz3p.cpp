// End-to-end test for RSS3 kEqz and addEqGate.
// Run three instances:
//   ./test_eqz3p 0
//   ./test_eqz3p 1 127.0.0.1
//   ./test_eqz3p 2 127.0.0.1
...
static constexpr int BASE_PORT = 13640;

template <typename T>
bool runType(int pid, Net3P& net) {
  constexpr size_t width = std::numeric_limits<T>::digits;
  const T high_bit = static_cast<T>(uint64_t{1} << (width - 1));
  const std::vector<T> x_vals = {
      T{0}, T{1}, T{5}, std::numeric_limits<T>::max(),
      high_bit, T{0}, T{17}, static_cast<T>(high_bit - T{1})};
  const std::vector<T> y_vals = {
      T{0}, T{2}, T{5}, T{1}, high_bit, T{9}, T{18},
      static_cast<T>(high_bit - T{1})};

  Circuit<T> circuit;
  std::vector<wire_t> x(x_vals.size()), y(y_vals.size());
  for (size_t i = 0; i < x.size(); ++i) x[i] = circuit.newInputWire(P0);
  for (size_t i = 0; i < y.size(); ++i) y[i] = circuit.newInputWire(P1);
  for (size_t i = 0; i < x.size(); ++i) {
    circuit.setAsOutput(circuit.addEqzGate(x[i]));
    circuit.setAsOutput(circuit.addEqGate(x[i], y[i]));
  }
  ...
  expected.reserve(2 * x_vals.size());
  for (size_t i = 0; i < x_vals.size(); ++i) {
    expected.push_back(x_vals[i] == 0 ? T{1} : T{0});
    expected.push_back(x_vals[i] == y_vals[i] ? T{1} : T{0});
  }

  const bool ok = outputs.vals == expected;
  std::printf("[P%d] RSS3 kEqz/equality uint%zu: %s\n",
